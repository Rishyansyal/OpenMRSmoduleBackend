...
We hebben een relationele database nodig die zowel ASP.NET Core Identity ([ADR 0003](0003-use-identity-framework-for-auth.md)) als onze domein-tabellen ([ADR 0020](0020-use-ef-core-for-persistence.md)) ondersteunt. De realistische opties: PostgreSQL, SQL Server, MySQL/MariaDB, SQLite.

## Decision

We gebruiken **PostgreSQL 17** voor zowel lokaal (Docker) als productie.

- Provider: `Npgsql.EntityFrameworkCore.PostgreSQL` voor Identity en domein-persistentie.
...
|---|---|
| **SQL Server** | Licentiekosten in productie; sterk Windows-leaning; minder neutrale keuze voor ziekenhuis-hosting op verschillende platforms. |
...
| **SQLite** | Geen concurrent writes onder load; geen geschikte productie-DB voor meerdere API-instanties. We gebruiken het wel als test-DB voor integratietests ([ADR-0013](0013-pragmatic-automated-test-strategy.md)). |
